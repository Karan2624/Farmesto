    ...
    <div className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-md border-t border-slate-200 px-2 py-2 flex justify-between items-center z-50 md:hidden shadow-[0_-4px_20px_rgba(0,0,0,0.05)] pb-safe">
      {
        navItems.map((item)=> {
          ...
          return (
            <Link key={item.href} href={item.href} className={`flex flex-col gap-1 items-center p-2 rounded-2xl transition-all duration-300 min-w-[3.5] ${
              isActive ? 'text-emerald-600 -translate-y-2' 
                ...
                <div>
                  <span className={`text-[10px] transition-opacity font-medium ${isActive ? 'opacity-100':'opacity-700'}`}>{item.label}</span>
                </div>
            </Link>
          )
        })
